...
Preferred setup: run the onboarding wizard (`edwinpai onboard`). It walks through gateway, workspace, channels, and skills. The CLI wizard is the recommended path and works on **macOS, Linux, and Windows (via WSL2; strongly recommended)**.
...
For a normal beta install, use npm:

```bash
npm install -g @edwinpai/edwinpai@beta
edwinpai onboard --install-daemon
edwinpai status
```

Or use the hosted installer once it is deployed:
...
The onboarding wizard walks through gateway, workspace, channels, and skills. `--install-daemon` installs the Gateway daemon (launchd/systemd user service) so it stays running.
...
# Talk to the assistant locally
edwinpai agent --message "Say hello from EdwinPAI" --thinking off
```

After onboarding, connect channels from the wizard or config, then test delivery with the relevant channel CLI/tool.

Upgrading? [Updating guide](https://docs.edwinpai.com/install/updating) (and run `edwinpai doctor`).
...
Switch channels (git + npm): `edwinpai update --channel stable|beta|dev`.
...
```bash
git clone https://github.com/edwinpai/edwinpai.git
cd edwin

pnpm install
pnpm build

pnpm edwinpai onboard --install-daemon

# Dev loop (auto-reload on TS changes)
pnpm gateway:watch
```

Note: `pnpm edwinpai ...` runs TypeScript directly (via `tsx`). `pnpm build` produces `dist/` for running via Node / the packaged `edwinpai` binary.
...
- Approve with: `edwinpai pairing approve <channel> <code>` (then the sender is added to a local allowlist store).
- Public inbound DMs require an explicit opt-in: set `dmPolicy="open"` and include `"*"` in the channel allowlist (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`).

Run `edwinpai doctor` to surface risky/misconfigured DM policies.

## Highlights

- **[Local-first Gateway](https://docs.edwinpai.com/gateway)** — runtime control plane for actors, sessions, channels, tools, events, and approval flows.
- **[Multi-channel inbox](https://docs.edwinpai.com/channels)** — communication media such as WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, BlueBubbles (iMessage), iMessage (legacy), Microsoft Teams, Matrix, Zalo, Zalo Personal, WebChat, macOS, iOS/Android.
- **[Multi-agent routing](https://docs.edwinpai.com/gateway/configuration)** — route inbound channel/account/peer events to isolated agents (workspaces + per-agent sessions) without treating channels as user roles.
...
- Desktop: [edwin-desktop](https://github.com/edwinpai/desktop)
...
- [GitHub](https://github.com/edwinpai/edwinpai)
